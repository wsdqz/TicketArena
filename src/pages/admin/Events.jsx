...
const Events = () => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await fetch(`http://localhost:5001/api/events?page=${page}&per_page=${perPage}`, {
        credentials: 'include'
      });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/events/${eventToDelete.id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
