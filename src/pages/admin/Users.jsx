...
const Users = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch(`http://localhost:5001/api/admin/users?page=${page}&per_page=${perPage}`, {
        credentials: 'include'
      });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = `http://localhost:5001/api/admin/users/${selectedUser.id}`;
